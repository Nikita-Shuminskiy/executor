...
export const authApi = {
	...
	async authWithGoogle(payload: AuthGooglePayload) {
		const URL =
			Platform.OS === 'ios' ? 'auth_executor_by_google' : 'auth_executor_by_google_for_android'
		return await instance.post(
			URL,
			{},
			{
	...
	async deletePhoto(photo_id: number) {
		return await instance.post(
			`delete_executor_photos_for_approval`,
			{},
			{
				params: { photo_id },
			}
	...
	async examNextQuestion(lang: string) {
		return await instance.get<ExamNextQuestionWithTotalResponseType>(
			`executor_exam_next_question`,
			{ params: { lang } }
		)
	},
	...
	async getExamAnswer(lang: string, question: string, answer: string) {
		return await instance.get<ExamEducationResponseType<'ok' | 'Wrong answer'>>(
			`executor_exam_answer`,
			{
				params: {
					lang,
					question,
	...
	async getSettingExecutorShort() {
		return await instance.get<ExecutorShortDataType>(`get_settings_executor_short`)
	},

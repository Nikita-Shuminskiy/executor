...
const PriceViewer = memo(({ onChangeValuesPrice, price, priceDataPayload }: any) => {
	...
	const getInfo = getInfoPriceElement(price?.id, dictionary)

	const onChangeValue = (val: string) => {
		onChangeValuesPrice(price.id, price, val)
	}
	return (
		<Box minH={104} p={3} style={styles.shadow} borderRadius={20}>
			...
			>
				<Text fontSize={13} color={colors.gray} fontFamily={'regular'}>
					{dictionary[DictionaryEnum.NumberOfItems]}
				</Text>

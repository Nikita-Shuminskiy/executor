...
const PhoneVerificationCode = observer(({ navigation, isFromUpdate }: PhoneVerificationProps) => {
	...
	useEffect(() => {
		if (code.trim().length === 5) {
			AuthStoreService.sendVerifyCode(code).then((data) => {
				...
				if (data.status === 'ok') {
					if (isFromUpdate)
						return navigation.navigate(routerConstants.PROFILE, { from: 'open_menu' })
	...
	return (
		<View>
			...
			/>
			{!isValid && (
				<Text mt={2} fontFamily={'regular'} fontSize={15} color={colors.red}>
					{dictionary[DictionaryEnum.IncorrectConfirmationCode]}
				</Text>
			)}
		</View>
	)

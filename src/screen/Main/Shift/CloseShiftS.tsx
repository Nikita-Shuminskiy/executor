...
const CloseShiftS = observer(({ navigation }: CloseShiftSProps) => {
	...
	return (
		<BaseWrapperComponent isKeyboardAwareScrollView={true}>
			<Box paddingX={4} mt={3}>
				<HeaderGoBackTitle
					title={dictionary[DictionaryEnum.SnoozeShiftsUntil]}
					goBackPress={goBack}
				/>
				<Text mt={4} fontSize={28} fontFamily={'semiBold'}>
					{dictionary[DictionaryEnum.Suspension]}
				</Text>
				<Text mt={2} fontSize={17} fontFamily={'regular'} color={colors.gray}>
					{dictionary[DictionaryEnum.SpecifyStopNotificationsDate]}
				</Text>
				{executorSettings?.executors?.datetime_freeze_until && (
					<Text
						...
						color={colors.blue}
					>
						{dateStringFormat(executorSettings?.executors?.datetime_freeze_until, 'dd MMMM yyyy')}
					</Text>
				)}
				<Box mt={3}>
					<DatePicker
				...
				<Box mt={3} w={'100%'}>
					...
					>
						...
						<Text ml={2} fontSize={15} color={colors.white} fontFamily={'semiBold'}>
							{dictionary[DictionaryEnum.SuspendWork]}
						</Text>
					</Button>
				</Box>
			</Box>

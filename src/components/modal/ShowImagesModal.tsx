...
const ShowImagesModal = ({ visible, onClose, image }: BaseModalProps) => {
	return (
		<Modal transparent={false} visible={visible}>
			<BaseWrapperComponent styleSafeArea={{ paddingTop: 10 }} isKeyboardAwareScrollView={false}>
				<Box pl={4} position={'relative'} bottom={3} top={2} zIndex={10} >
					<HeaderGoBackTitle goBackPress={onClose} title={''} />
				</Box>
				<Box paddingY={4}>
					<Box mt={2}>
						<Image resizeMethod={'auto'} resizeMode={'cover'} style={{ width: '100%', height: '100%' }} source={{ uri: image }} />
					</Box>
				</Box>
